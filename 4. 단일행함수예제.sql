...
INSERT INTO varchar_compare VALUES ('101', 'SQLD', 'SQLD  ');--의도적으로 공백 넣어야 들어감
...
SELECT 
    ...
    SUBSTR('SQL Developer', 1, 3), -- 문자열 자름 1번부터 3개자름 (첫글자가 1번), 중요
    LENGTH('HELLO WORLD'),         -- 문자열의 길이, 글자 수 세기, 공백포함
    TRIM('    HI   ')              -- 좌우 공백 제거
FROM dual;
...
SELECT 
    ...
    REPLACE('Java Programmer', 'Java', 'BigData') AS "REPLACE" -- 자바를 빅데이터로 문자를 변경
    ,REPLACE('Java Programmer', 'Java') AS "REPLACE" -- 자바문자를 제거
FROM dual;
...
SELECT 
    ...
    (SYSDATE + 10) - SYSDATE AS "날짜 - 날짜",--차이나는 일수가 나옴
...
-- 시간 포맷형식: HH12 - 시(1~12)--낮밤구분해줘야 함,AM이라고 쓰면 알아서 구분
--HH24 (0-23), MI - 분, SS - 초
SELECT
    SYSDATE,--M은 꼭 두개 써야 함..
...
SELECT
    TO_CHAR(9512 * 1.33, '$999,999.99') AS "달러",--9는 그냥 상징적인 숫자....
    TO_CHAR(1350000, '999,999,999L') AS "원화"--L은 지역화폐
FROM dual;
...
SELECT 
    sal_cd, sal_nm,
    CASE WHEN sal_cd = '100001' THEN '기본급여' --조건식
...
FROM tb_emp
WHERE emp_nm = '박찬호';--없어서 안나옴, 공집합

SELECT 
    MAX(emp_nm),
    NVL(MAX(emp_nm), '존재안함') AS emp_nm--이건 null일경우에 쓰는 함수라 max를 써줌
    --없으니까 공집합이 들어가서 max가 null을 뱉어냄
FROM tb_emp
WHERE emp_nm = '박찬호';
